...
const HOMEPAGE_FILM_URL_KEY = "homepage_film_url";
...
/**
 * Public read: a video uploaded in Studio > Homepage for the mid-page film
 * section. When set it replaces the Vimeo embed; empty means use Vimeo.
 */
export async function getHomepageFilmUrl(): Promise<string | null> {
  try {
    if (!supabase) return null;
    ...
    const { data, error } = await supabase
      .from("platform_settings")
      .select("value")
      .eq("key", HOMEPAGE_FILM_URL_KEY)
      .maybeSingle();

    if (error) throw error;
    return data?.value?.trim() || null;
  } catch (error) {
    console.error("Error fetching homepage film url:", error);
    return null;
  }
}
