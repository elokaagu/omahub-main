...
/**
 * Every designer application, newest first, matched to the brand it created
 * (same name + contact email) so the list can show "approved -> brand".
 * Pass a service-role client; callers must have checked for a super admin.
 * Throws if the applications query fails.
 */
export async function loadStudioApplications(supabase: SupabaseClient) {
