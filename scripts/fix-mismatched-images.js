...
async function fixMismatchedImages() {
  try {
    ...
    console.log(
      `🖼️ Available original brand images: ${availableImages.length}`
    );
    ...
    for (let i = 0; i < brands.length; i++) {
      ...
      if (i < availableImages.length) {
        ...
        newImageUrl =
          "https://via.placeholder.com/400x600/f3f4f6/9ca3af?text=Image+Coming+Soon";
    ...
    if (productsError) {
      ...
      for (let i = 0; i < products.length; i++) {
        ...
        if (updateError) {
          console.error(
            `❌ Failed to update product ${product.title}:`,
            updateError
          );
        } else {
          console.log(
            `✅ Updated product ${product.title} with collection image`
          );
        }
      }
    }
