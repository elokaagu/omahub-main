...
async function fixBrandImageMatching() {
  try {
    ...
    console.log(
      `🗺️ Created mapping for ${Object.keys(brandImageMapping).length} brands`
    );
    ...
    for (const brand of brands) {
      ...
      if (correctImageName) {
        ...
        const imageExists = imageFiles.find(
          (file) => file.name === correctImageName
        );
        ...
        if (imageExists) {
          ...
          console.log(`\n🔗 Updating ${brand.name}:`);
          console.log(
            `   Current image: ${brand.image ? brand.image.substring(0, 80) + "..." : "None"}`
          );
          ...
          console.log(
            `⚠️ Image file not found for ${brand.name}: ${correctImageName}`
          );
          errorCount++;
        }
      } else {
    ...
    if (!verifyError && updatedBrands) {
      const brandsWithRealImages = updatedBrands.filter(
        (b) => b.image && !b.image.includes("placeholder")
      );

      console.log(
        `✅ ${brandsWithRealImages.length} brands now have real images`
      );
      console.log(
        `📝 ${updatedBrands.length - brandsWithRealImages.length} brands still have issues`
      );
      ...
      updatedBrands.slice(0, 10).forEach((brand) => {
        ...
        console.log(
          `   ${brand.name}: ${hasRealImage ? "✅ " + imageName : "❌ " + imageName}`
        );
      });
    }
