-- Ultra-simple RLS policies for products
-- Run this entire script in Supabase SQL Editor

-- Drop all existing policies
DROP POLICY IF EXISTS "view_products" ON public.products;
DROP POLICY IF EXISTS "super_admin_access" ON public.products;
DROP POLICY IF EXISTS "brand_admin_access" ON public.products;
...
-- Anyone can view
CREATE POLICY "view_all" ON public.products FOR SELECT USING (true);

-- Super admins can do everything
CREATE POLICY "super_all" ON public.products FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'super_admin')
);

-- Brand admins can do everything on their products
CREATE POLICY "brand_own" ON public.products FOR ALL USING (
  EXISTS (
