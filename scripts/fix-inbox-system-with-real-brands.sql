-- Complete Inbox System Setup (Fixed Version with Real Brand IDs)
...
-- 18. Insert sample data using real brand IDs from the database
-- This will only insert if brands exist
DO $$
DECLARE
    brand_ids TEXT[];
BEGIN
    ...
    IF array_length(brand_ids, 1) >= 1 THEN
        ...
        (brand_ids[1], 'Amara Johnson', 'amara.johnson@email.com', '+234 803 123 4567', 'Custom Design Order', 'Hi! I love your designs. I would like to place a custom order for a special occasion dress. What are your available options and pricing?', 'custom_order', 'high', 'website')
        ON CONFLICT DO NOTHING;
    END IF;
    
    IF array_length(brand_ids, 1) >= 2 THEN
        INSERT INTO public.inquiries (brand_id, customer_name, customer_email, customer_phone, subject, message, inquiry_type, priority, source) VALUES
        (brand_ids[2], 'Chioma Okafor', 'chioma.okafor@email.com', '+234 901 234 5678', 'Product Inquiry', 'Hello, I am interested in your products and would like to know more about your collections. Can you provide details about pricing and availability?', 'general', 'normal', 'whatsapp')
        ON CONFLICT DO NOTHING;
    END IF;
    
    IF array_length(brand_ids, 1) >= 3 THEN
        INSERT INTO public.inquiries (brand_id, customer_name, customer_email, customer_phone, subject, message, inquiry_type, priority, source) VALUES
        (brand_ids[3], 'Fatima Abdullahi', 'fatima.abdullahi@email.com', '+234 812 345 6789', 'Wholesale Inquiry', 'Good day! I run a business and would like to inquire about wholesale pricing for your products. Do you offer bulk discounts?', 'wholesale', 'normal', 'website')
        ON CONFLICT DO NOTHING;
    END IF;
END $$;
...
-- Show what brands exist
SELECT 'Available brands:' as info, id, name FROM public.brands LIMIT 5;
