...
async function fixSpecialCharacterBrand() {
  try {
    ...
    const specialBrand = brands.find(b => 
      b.name.includes("LENTAR") || 
      b.name.includes("LÈNTÁR") ||
      b.name.includes("K") && b.name.includes("L") && b.name.includes("N") && b.name.includes("T") && b.name.includes("R")
    );
    ...
    console.log(`   Current image: ${specialBrand.image ? specialBrand.image.substring(0, 80) + '...' : 'None'}`);
    ...
    if (updateError) {
      ...
      if (!verifyError && updatedBrand) {
        ...
        console.log(`   Has real image: ${updatedBrand.image && !updatedBrand.image.includes("placeholder") ? "✅ Yes" : "❌ No"}`);
      }
    }
