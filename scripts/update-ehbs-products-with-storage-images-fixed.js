...
async function updateEhbsProductsWithStorageImages() {
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      // Update the product with new image (only the main image field)
    ...
    // Show the image mapping
    console.log("\n📸 Image assignments:");
    verifyProducts.forEach((product, index) => {
      const imageIndex = index % availableImages.length;
      const assignedImage = availableImages[imageIndex];
      console.log(`   ${product.title} → ${assignedImage.split("/").pop()}`);
    });
  } catch (err) {
