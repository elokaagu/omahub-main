...
async function findAllBrandImages() {
  try {
        ...
        try {
          ...
          if (listError) {
            ...
            if (imageFiles.length > 0) {
              console.log(
                `   🖼️ Found ${imageFiles.length} potential image files:`
              );
    ...
    try {
      ...
      if (columnsError) {
        ...
        if (!sampleError && sampleBrand && sampleBrand[0]) {
          ...
          const imageColumns = brandColumns.filter(
            (col) =>
              col.includes("image") ||
              col.includes("logo") ||
              col.includes("photo") ||
          ...
          if (imageColumns.length > 0) {
            console.log(
              `   🖼️ Potential image columns found: ${imageColumns.join(", ")}`
            );
          }
        }
      } else {
    ...
    try {
      ...
      if (tablesError) {
        console.log(
          "   ⚠️ Couldn't list all tables, checking common backup names..."
        );
        ...
        for (const tableName of backupTableNames) {
          try {
            ...
            if (!backupError && backupData && backupData.length > 0) {
              ...
              console.log(
                `      Sample data: ${backupData[0].name} - ${backupData[0].image}`
              );
            }
          } catch (e) {
        ...
        const backupTables = tables.filter(
          (t) =>
            ...
            (t.table_name.includes("backup") ||
              t.table_name.includes("archive") ||
              t.table_name.includes("old"))
        );

        if (backupTables.length > 0) {
          console.log(
            `   🗂️ Found backup tables: ${backupTables.map((t) => t.table_name).join(", ")}`
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (!brandsError && brands) {
        console.log(
          `   📋 Checking ${brands.length} brands for embedded data...`
        );

        // Look for brands with very long image URLs (might be base64)
        const longImageBrands = brands.filter(
          (b) => b.image && b.image.length > 200
        );

        if (longImageBrands.length > 0) {
          console.log(
            `   🔍 Found ${longImageBrands.length} brands with long image data:`
          );
          longImageBrands.forEach((brand) => {
            console.log(
              `      ${brand.name}: ${brand.image.substring(0, 100)}...`
            );
          });
        }

        // Check for brands with recent updates (might have lost images)
        const recentBrands = brands.filter(
          (b) => b.updated_at && new Date(b.updated_at) > new Date("2025-01-01")
        );

        if (recentBrands.length > 0) {
          console.log(
            `   📅 Found ${recentBrands.length} brands updated in 2025:`
          );
        ...
        for (const column of allColumns) {
          if (
            ...
            if (columnValues.length > 0) {
              console.log(
                `   🔗 Column '${column}' contains URLs: ${columnValues.join(", ")}`
              );
            }
          }
        }
