...
async function restoreOriginalBrandImages() {
  try {
    console.log(
      "🎯 Restoring original brand images from product-images bucket..."
    );
    ...
    console.log(
      `🖼️ Found ${productImages.length} images in product-images bucket`
    );
    ...
    if (brands.length > imageFiles.length) {
      ...
      console.log(
        "   These brands will keep their current images (placeholders or collection images)"
      );
    }
    ...
    if (!verifyError && updatedBrands) {
      ...
      console.log(
        `📝 ${updatedBrands.length - brandsWithRealImages.length} brands still have placeholders`
      );
      ...
      updatedBrands.slice(0, 5).forEach((brand) => {
        ...
        console.log(
          `   ${brand.name}: ${hasRealImage ? "✅ Real image" : "📝 Placeholder"}`
        );
      });
    }
