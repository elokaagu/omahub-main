...
// Safe collections mapping with fallback
...
export default function Header() {
  ...
  useEffect(() => {
    async function loadDynamicNavigation() {
      try {
        ...
        // Safety check for dynamic items
        if (!dynamicItems || !Array.isArray(dynamicItems)) {
          console.warn("Header: Invalid dynamic items, using fallback");
  ...
  // Safe navigation rendering function
  const renderNavigationItems = () => {
    ...
    if (!dynamicNavigationItems || !Array.isArray(dynamicNavigationItems)) {
      return null;
    }

    return dynamicNavigationItems.map((category) => {
      // Safety check for category and items
    ...
    >
                      ...
                      {(user?.role === "admin" ||
                        ...
                        <li>
                          <button
                            ...
                            className="flex items-center gap-3 w-full rounded-md px-3 py-2.5 text-sm font-medium text-oma-plum hover:bg-oma-beige/50 transition-colors disabled:opacity-50"
                          >
      ...
      {mobileMenuOpen && (
        ...
        >
          ...
          {/* Menu panel */}
          <div className="fixed inset-y-0 right-0 z-[1002] w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
                  ...
                  {user ? (
                    ...
                    <div className="space-y-3">
                      ...
                      <NavigationLink
                        href="/login"
                        onClick={() => setMobileMenuOpen(false)}
                        className="-mx-3 flex items-center gap-3 rounded-lg px-3 py-3 text-base font-semibold leading-7 text-oma-plum hover:bg-oma-beige/50 transition-colors"
                      >
                        <span>Sign In</span>
                      </NavigationLink>
