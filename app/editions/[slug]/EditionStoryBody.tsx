  ...
  switch (block.type) {
    ...
    case "paragraph":
      return (
        <p
          className={cn(
            "mt-5 font-canela leading-relaxed sm:mt-6",
            isDek
              ? "text-xl text-oma-cocoa sm:text-2xl"
              : "text-lg text-oma-black sm:text-xl",
          )}
    ...
    case "list":
      return (
        <div
          className="edition-story-list mt-6 font-canela text-lg leading-relaxed text-oma-black sm:mt-7 sm:text-xl [&_li]:mt-2.5 [&_li]:pl-1 [&_ol]:list-decimal [&_ol]:pl-6 [&_ul]:list-none [&_ul>li]:relative [&_ul>li]:pl-5 [&_ul>li]:before:absolute [&_ul>li]:before:left-0 [&_ul>li]:before:top-[0.7em] [&_ul>li]:before:h-1.5 [&_ul>li]:before:w-1.5 [&_ul>li]:before:rounded-full [&_ul>li]:before:bg-oma-plum"
          dangerouslySetInnerHTML={{ __html: block.html }}
        />
      );
    case "quote":
      return (
        <blockquote
          className="my-10 border-l-2 border-oma-gold pl-5 font-canela text-xl italic leading-relaxed text-oma-cocoa sm:my-12 sm:pl-6 sm:text-2xl"
          dangerouslySetInnerHTML={{ __html: block.html }}
        />
      );
    case "images":
