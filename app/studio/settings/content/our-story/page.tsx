...
export default function OurStoryEditorPage() {
  ...
  useEffect(() => {
    ...
    const fetchContent = async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq("key", "our_story")
        .single();
      if (data) setOurStory(data.content || "");
      setLoading(false);
    };
    fetchContent();
  }, []);
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      .from("studio_content")
      .upsert({ key: "our_story", content: ourStory });
    setSaving(false);
    if (error) {
      toast({
        title: "Failed to save Our Story",
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(ourStory);
      ...
      <Card>
        ...
        <CardContent>
          <textarea
            ...
            placeholder="Write your Our Story paragraph here..."
            disabled={loading}
          />
