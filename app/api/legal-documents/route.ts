...
import { defaultLegalDocuments } from "@/lib/legal/defaultLegalDocuments";
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error && error.code === "PGRST116") {
      ...
      let filteredDocuments = defaultLegalDocuments;

      if (documentType) {
        filteredDocuments = defaultLegalDocuments.filter(
